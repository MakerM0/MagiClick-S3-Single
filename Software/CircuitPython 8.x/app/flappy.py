...
score_area.anchored_position = (display.width-1, 0)

game_over_text.anchor_point = (0.5, 0.5)
game_over_text.anchored_position = (display.width//2, display.height//2)

new_game_text.anchor_point = (0.5, 0.5)
new_game_text.anchored_position = (display.width//2, display.height//2)
...
while True:
    ...
    if now>(old_pipe+0.03) and new_game==True:
        ...
        if pipe_down_grid.x<-13:
            pipe_down_grid.x=display.width
            pipe_down_grid.y=random.randrange(-60,0,5)
            pipe_up_grid.x=display.width
            pipe_up_grid.y=pipe_down_grid.y+pipe_distance+80
