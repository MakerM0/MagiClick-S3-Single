...
# FREQUENCYS = [196,207,220,349,493,523,783] # Set this to the Hz of the tone you want to generate.
# audio = audiobusio.I2SOut(AUDIO_BCK,AUDIO_WS,AUDIO_DATA)
# 
# 
# sine_waves=[]
# sine_waves_sample=[]
...
#     sine_waves_sample.append(sine_wave_sample)
# 
# 
# 
# def playwave(index,t):
#     pass
#      
#     audio.play(sine_waves_sample[index], loop=True )
#     time.sleep(t*100)
#     audio.stop()
...
#                    max_events=1)
pad = None
latest_event =keypad.Event() #Event(key_number=8)
last_read = 0

# imageFile = "/spritesNew.bmp"
imageFile = "/images/game/dino.bmp"
...
sparky0_grid.x = 100
sparky0_grid.y = 32 + OFFSET_Y

sparky0_group = displayio.Group()
sparky0_group.append(sparky0_grid)

#  2nd Sparky sprite
sparky1, sparky1_pal = adafruit_imageload.load(imageFile,
                                               bitmap=displayio.Bitmap,
                                               palette=displayio.Palette)
...
sparky1_grid = displayio.TileGrid(sparky1, pixel_shader=sparky1_pal,
                                  ...
                                  default_tile=SPARKY)
sparky1_grid.x = 100
sparky1_grid.y = 32 + OFFSET_Y

sparky1_group = displayio.Group()
sparky1_group.append(sparky1_grid)

#  3rd Sparky sprite
sparky2, sparky2_pal = adafruit_imageload.load(imageFile,
                                               bitmap=displayio.Bitmap,
                                               palette=displayio.Palette)
...
sparky2_grid = displayio.TileGrid(sparky2, pixel_shader=sparky2_pal,
                                  ...
                                  default_tile=SPARKY)
sparky2_grid.x = 100
sparky2_grid.y = 32 + OFFSET_Y

sparky2_group = displayio.Group()
sparky2_group.append(sparky2_grid)

#  heart sprite group
life_bit, life_pal = adafruit_imageload.load(imageFile,
                                             bitmap=displayio.Bitmap,
                                             palette=displayio.Palette)
...
while True:
    ...
    if not game_over and not new_game:
        ...
        if snake:
            ...
            for s in range(3):
                if sparky_x[s] == 8 and blinka_grid.y == 32+OFFSET_Y:
#                     audiopwr_on()
#                     playwave(4,0.005)
#                     playwave(3,0.005)                    
#                     audiopwr_off()
